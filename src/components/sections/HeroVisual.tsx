...
export function HeroVisual() {
  ...
  return (
    <div className="relative mx-auto aspect-[600/560] w-full max-w-[620px]">
      {/* ambient glow */}
      <div aria-hidden className="absolute inset-[15%] rounded-full bg-[radial-gradient(closest-side,rgb(var(--accent-rgb)/0.22),transparent)] blur-2xl" />
      ...
      <svg viewBox="0 0 620 560" className="relative h-full w-full" role="img" aria-label="Pixel ADX advertising ecosystem: advertisers, ad platforms, audiences, data and conversions connected through Pixel ADX">
        <defs>
          <linearGradient id="hv-line" x1="0" y1="0" x2="1" y2="0">
            <stop offset="0" style={{ stopColor: "var(--color-cyan)" }} stopOpacity="0.5" />
            <stop offset="1" style={{ stopColor: "var(--color-violet)" }} stopOpacity="0.5" />
          </linearGradient>
          ...
          <radialGradient id="hv-hub">
            ...
            <stop offset="0.6" style={{ stopColor: "var(--color-accent)" }} stopOpacity="0.12" />
        ...
        {links.map(([f, t, k], i) => {
          ...
          return (
            <g key={id}>
              ...
              {!reduce &&
                [0, 1].map((n) => (
                  <circle key={n} r={n === 0 ? 3 : 2} style={{ fill: n === 0 ? "#9cc2ff" : "var(--color-cyan)" }}>
        ...
        {nodes.map((n) =>
          n.hub ? (
            <g key={n.id}>
              ...
              <circle cx={n.x} cy={n.y} r="46" fill="none" strokeOpacity="0.6" className={reduce ? "" : "animate-pulse-soft"} style={{ stroke: "var(--color-accent)", transformOrigin: `${n.x}px ${n.y}px` }} />
            ...
            <g key={n.id}>
              ...
              <circle cx={n.x} cy={n.y} r="9" fill="none" style={{ stroke: "var(--color-cyan)" }} strokeOpacity="0.5" />
